...
async function dislikeMessage(req, res, next) {
  ...
  try {
    ...
    if (message.dislikedBy.includes(login)) { // Vérifie si l'utilisateur a déjà disliké ce message
    ...
    message.dislikedBy.push(login); // Ajoute le nom de l'utilisateur à la liste des utilisateurs ayant disliké le message
